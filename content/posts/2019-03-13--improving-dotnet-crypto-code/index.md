...
> This is the first (and maybe least interesting) post of a small series on AES-NI and .NET Core hardware intrinsics. The [next post](https://ndportmann.com/aes-ni-key-expansion/ "AES-NI key expansion - ndportmann.com") starts to take a deep dive into AES-NI. This one is more about efficiently using the existing crypto APIs in .NET.

## Motivation

I was researching [AES New Instructions (AES-NI)](https://www.intel.com/content/dam/doc/white-paper/advanced-encryption-standard-new-instructions-set-paper.pdf "AES-NI Whitepaper - Intel") and the support for [hardware intrinsics in .NET Core 3.0](https://devblogs.microsoft.com/dotnet/using-net-hardware-intrinsics-api-to-accelerate-machine-learning-scenarios/ ".NET Hardware Intrinsics - Microsoft Devblog") when i stumbled over [this repository](https://github.com/ronnieholm/Playground/tree/master/TrustpilotAesChallenge "TrustpilotAesChallenge - ronnieholm - GitHub") comparing different implementations of a [coding challenge](http://followthewhiterabbit.trustpilot.com/challenge2.html "Trustpilot - Coding Challenge") from [Trustpilot](https://www.trustpilot.com/ "Trustpilot"). What puzzled me most, was the fact that the .NET Core implementation took 6 minutes to complete, while an implementation in C only required 9 seconds (a factor of 40 slower!). I wouldn't have been surprised if a well optimized native implementation of a given problem is faster than the equivalent managed implementation in C#, but the factor was just way too high. So I started to tweak the code (first a little, then a LOT) and ended up with a solution that outperformed the implementation in C. However, this post is only about the first ~9x improvement, with more to follow as promised above.
...
So I was happy for the day and opened a [PR](https://github.com/ronnieholm/Playground/pull/1 "Pull Request #1 - Playground - ronnieholm - GitHub") but I also realized, that the frameworks AES implementation isn't really designed to perform crypto operations on relatively small messages using constantly changing keys. The [next post](https://ndportmann.com/aes-ni-key-expansion/ "AES-NI Key Expansion - ndportmann.com") of this series focuses on how to work around exactly that issue.